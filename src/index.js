...
function App() {
    ...
    var createEvent = function () {
        ...
        //监听touchstart事件
        // 第一次touchstart时设定计时器，如果在指定的时间内没有其它操作，就认为是单击，dispatchEvent(oneTouch)
        // 如果在指定的时间内，收到第二次touchstart事件，并且与上次点击的时间间隔在250ms内，就认为是双击
        $bm.addEventListener(
            "touchstart",
            function (e) {
                //e.preventDefault();
        ...
        function gesturef(event) {
            switch (event.type) {
                ...
                case "gesturechange":
                    ...
                    tMatrix[0] =
                        tMatrix[0] + sc - 1 > 0.1 && tMatrix[0] + sc - 1 < 30
                            ? tMatrix[0] + sc - 1
                            : tMatrix[0];
                    tMatrix[3] =
                        tMatrix[3] + sc - 1 > 0.1 && tMatrix[3] + sc - 1 < 30
                            ? tMatrix[3] + sc - 1
                            : tMatrix[3];
    ...
    return (
        <div className="App">
            <h4>点击图片预览</h4>
            <img class="img1 preview" src="http://lc-yee5xnhu.cn-n1.lcfile.com/56ac839df83d3bcdc63b.jpg" alt="" />
        </div>
    );
}
